...
// 获取所有品牌
export function initBrandFilter(post: Post[]) {
    const data: any = {}
    for (let index = 0; index < post.length; index++) {
        const element = post[index]
        let brands = []
        if (element?.frontmatter?.brand) {
...
export function initBrands(post: Post[]) {
    ...
    for (let index = 0; index < post.length; index++) {
        const element = post[index]
        let brands = []
        if (element?.frontmatter?.albums) {
            brands = element.frontmatter.albums
        } else if (element?.frontmatter?.album) {
            brands = Array.isArray(element.frontmatter.album) ? element.frontmatter.album : [element.frontmatter.album]
        }
        if (brands && brands.length > 0) {
            brands.forEach((item:any) => {
                if (!item) return
