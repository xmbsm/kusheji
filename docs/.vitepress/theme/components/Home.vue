...
const posts = computed(() => {
  if (selectCategory.value) {
    ...
    return themeposts.filter((article: any) => {
      const brands = article?.frontmatter?.albums || (article?.frontmatter?.album ? (Array.isArray(article.frontmatter.album) ? article.frontmatter.album : [article.frontmatter.album]) : [])
...
router.onBeforeRouteChange = (to) => {
  ...
  activeBrand.value = params?.album || ''
...
watch(
  ...
  () => {
    ...
    if (location.value.href) {
      ...
      activeBrand.value = url.searchParams.get('album') || ''
